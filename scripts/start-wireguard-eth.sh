...
set -u
CONFIG=wg-eth
SOURCE_CONFIG="$HOME/client-eth-macos-intel.conf"
...
ping -c 3 -W 1 "$TARGET" >/tmp/wireguard-eth-connectivity.log 2>&1; ping_rc=$?
...
if (( ping_rc == 0 && rx1 > rx0 && tx1 > tx0 )); then
    echo "WireGuard eth0 verified: RX $rx0 -> $rx1, TX $tx0 -> $tx1 bytes."
    exit 0
fi
echo "ERROR: verification failed; RX $rx0 -> $rx1, TX $tx0 -> $tx1; ping exit $ping_rc." >&2
cat /tmp/wireguard-eth-connectivity.log >&2
exit 1
